  ...
  이것은 `ConnectivityManager`를 활용하여 구현된 핵심 로직입니다.

    1. 모듈 목록 가져오기: `SQLiteModuleRepository`를 통해 `SYS_MODULE_MST`에서 모든 활성 모듈 검색
    2. 모듈 반복: 각 모듈별로 서버와 비교 수행:
        * A. Staging에 다운로드 (업데이트 확인):
            * 대상: `%AppData%\nU3.Framework\Cache\[SubSystem]\[FileName]`
            * 확인: `HttpDBAccessClient`를 통해 서버의 버전/해시 정보를 조회하여 로컬과 비교
            * 동작: 불일치 시 `HttpFileTransferClient`를 사용하여 서버에서 Staging 영역으로 다운로드
        * B. Runtime에 설치 (배포):
            * 대상: `[ExeDirectory]\Modules\[SubSystem]\[FileName]`
            * 확인: Runtime 폴더에 파일이 누락되었거나 Staging에 새 버전이 있는 경우
            * 동작: Staging에서 Runtime으로 파일 복사 (Overwrite)
            * 이점: Shell 실행 전이므로 파일 잠금(Locking) 없이 안전하게 교체 가능
