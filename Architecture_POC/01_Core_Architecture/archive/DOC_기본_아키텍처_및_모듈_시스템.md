# /sg:workflow [Phase1] 기본 아키텍처 및 모듈 시스템
  ...
  상태: **완료됨**
    ...
    - [x] `Core.dll` 어셈블리 정의:
        - [x] IBaseWorkControl 인터페이스 생성 (화면 식별 및 기본 정보 계약)
        - [x] nU3ProgramInfoAttribute 클래스 구현 (메타데이터 기반 자동 discovery 지원)
        - [x] UserSession 싱글톤 정의 (JWT 기반 전역 상태 관리)
        - [x] ConnectivityManager 싱글톤 정의 (인프라 서비스 중계자)

    - [x] 기본 폼 구현 (`nU3.Core.UI`):
        - [x] BaseWorkControl: UserControl 상속 + IBaseWorkControl + Lifecycle 관리
        - [x] BaseWorkForm: DevExpress Form 상속
        - [x] BaseWorkControl에 권한 확인(CanRead, CanUpdate 등) 및 리소스 관리(RegisterDisposable) 추가
  ...
  상태: **완료됨**
    ...
    - [x] 리플렉션 및 레지스트리 서비스 (`nU3.Shell`):
        ...
        - [x] MetadataScanner 구현: 로드된 DLL에서 [nU3ProgramInfo] 속성 스캔
        - [x] ProgramRegistry 구현: ProgramId (String) → Type (Class) 매핑
