  🎯 nU3.Framework (의료 시스템 UI Framework) 엔터프라이즈 구축 가이드

  - 언어: C# 12.0
  - Platform: WinForms (MDI)
  - Framework: .NET 8.0
  - UI Component: DevExpress WinForms v23.2.9
  - 핵심 설계 패턴: 
    - **Plugin Architecture**: AssemblyLoadContext 기반 동적 모듈 로딩
    - **Local-First**: SQLite 기반 클라이언트 메타데이터 관리
    - **Mediator Pattern**: EventAggregator를 통한 모듈 간 약결합 통신
    - **Service Agent**: ConnectivityManager를 통한 인프라(DB/File/Log) 통신 추상화
  ...
  ### 1. Data Access Layer (DAL) 구현 현황

  **현황:** `nU3.Connectivity` 및 `ConnectivityManager`를 통해 REST API 기반의 데이터 통신 레이어가 구현 완료되었습니다.
  
  *   **중앙 관리**: `ConnectivityManager.Instance` 싱글톤을 통해 모든 인프라 서비스에 접근합니다.
  *   **구체 구현체**: `HttpDBAccessClient`, `HttpFileTransferClient`, `HttpLogUploadClient`가 실제 서버(`nU3.Server.Host`)와 통신합니다.
  *   **보안**: JWT 기반 인증 토큰을 모든 요청 헤더에 자동으로 주입합니다.
  *   **성능**: 대량 작업을 위한 배치 처리(`ExecuteBatchQueriesAsync`) 및 동시 연결 수 제어(SemaphoreSlim) 기능이 포함되어 있습니다.
